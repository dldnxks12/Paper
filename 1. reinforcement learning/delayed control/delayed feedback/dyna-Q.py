...
One way to estimate the transition probability is to count the number of times that a given action a was taken in state s, 
and then divide this count by the total number of times that state s was visited. 
This gives us an estimate of the probability of transitioning from state s to the next state s' when action a is taken.


N_sa = np.zeros((num_states, num_actions))
N_sas = np.zeros((num_states, num_actions, num_states))

# Collect experience to estimate transition probabilities
...
즉, s,a,s_를 s,a로 나눠주면 transition probability가 된다!! 
...
class Model():
  ...
  def add(self,s,a,s_prime,r):
    self.transitions[s,a] = s_prime   # Deterministic env라고 가정
    self.rewards[s,a] = r
